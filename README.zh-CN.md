...
- **"刚才那个为什么失败了?"** 输出已经被 shao 录下来了,助手可以直接看那条失败的命令和它一字不差的输出,不需要你从早已滚走的屏幕里翻出报错再粘贴一遍。
- **"这东西该怎么部署?"** shao 能读取目标主机的实际情况——剩余磁盘、内存、哪些端口被占了、docker 和 nginx 现在在跑什么——所以建议是基于那台机器给的,而不是泛泛而谈。助手写命令,**你**来执行。

## 它和别的方案不一样在哪

已经有一些 MCP 服务器能给助手一个终端。**它们解决的是相反的问题**:给模型一个它自己的 shell,让它在里面执行命令。shao 是让它读**你的**终端,而且它什么都执行不了。由此衍生出五点差别。
...
**只读是 sshd 强制的,不是我们自觉。** 这个约束不是"我们没给 AI 注册执行工具"这种软的东西——它是挂在密钥上的 forced command,sshd 会**丢掉** SSH 客户端要求执行的任何内容。**即使这一侧被完全攻破,它依然成立。** 而 `shao host verify` 是靠**真的去试**来证明它:执行一条命令、读 `/etc/passwd`、创建文件、申请终端、转发端口——五项全部被拒绝才算通过。
...
**你不问,就什么都不跑。** 没有定时器、没有监视、没有告警、没有后台任务。在你两次提问之间,shao 只是一个往磁盘写东西的录制器,别的什么都不是——这也是为什么它没有什么需要"关掉"。
...
**AI 能读、不能执行——这由操作系统保证,而不是靠信任。** 对远程主机,shao 持有的那把 SSH 密钥是以 *forced command* 的形式装到目标机上的:sshd 会丢掉 SSH 客户端要求执行的任何东西,改为运行一个固定的只读探针。探针只接受一份**封闭列表**里的动词,不接受任何自由形式的参数,所以整条链路上没有任何一个字符串能让指令钻进去。**即使这一侧完全被攻破,这个约束依然成立。** 无法按这种方式配置的主机会被标记为 `disabled`,根本无法查询——这里刻意没有留"shao 保证自己会守规矩"这种弱化模式。

## 采集是怎么做的,以及为什么不用截图

shao 把你的 shell 跑在一个伪终端(pty)里,录下穿过它的字节流。shell 写出的每一个字节都恰好经过那个管道一次,顺序不变。**没有任何采样**,所以滚动速度快到你根本来不及看的输出也被完整记录;十分钟后再来问,拿到的是完整画面,而不是"两次采样瞬间屏幕上刚好有什么"。
...
**唯一真实的限制:** 录制从 shell 启动的那一刻开始。**已经开着的终端窗口无法追溯录制**——任何无损方法都做不到这件事。所以 `shao start` 一次覆盖两头:它录制你敲命令的这个终端,同时装一个启动钩子,让之后打开的每个终端自己就开始录。第一条命令之后,再没有任何"按终端"要做的事。

## 安装

shao 是一个单文件二进制,没有运行时依赖。下载、放到 `PATH` 上,完事。
...
`PATH` 上时提示你。用 `SHAO_INSTALL_DIR` 改安装位置,用
`SHAO_VERSION=v0.1.0` 锁定版本。
...
| macOS(Apple 芯片) | `shao_darwin_arm64.tar.gz` |
...
大多数发行版已经把 `~/.local/bin` 放在 `PATH` 上了。如果 `command -v shao` 是空的,加一行:

```sh
echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.bashrc
```

想让整台机器都能用,改成 `sudo install -Dm755 shao /usr/local/bin/shao`。
...
**第一次运行会被 Gatekeeper 拦下**,因为这些二进制没有用 Apple 开发者证书签名。macOS 会报成*"shao 已损坏,无法打开"*——这个提示是误导性的:文件没坏,只是带了隔离标记。清掉它:
...
| | |
|---|---|
| Windows | 10 1809 / Server 2019 及以上——更早的版本没有 ConPTY,而 shao 正是通过它采集的 |
| macOS | 11 及以上 |
...
```sh
shao start                 # 开
shao start --port 7337     # ……并顺便在一个 URL 上提供 MCP,如果你的客户端需要
shao end                   # 关
```

`shao start` 一次把必须发生的三件事都做了,免得你逐个去发现:

1. 让**之后打开的每个终端**都自己录制(改一处 shell 启动文件,改前先备份,用 `shao hook uninstall` 撤销)
2. 打印粘贴给 AI 客户端的配置——**一次**,不是每个终端一次
3. 录制**当前这个**终端,否则你敲命令的这扇窗口恰好会是唯一的盲区

然后照常干活,问你的 AI:*"刚才那个为什么失败了?"*

`shao end` 是它精确的反操作:停止覆盖新终端,并停掉正在跑的录制。**已经录下来的一切仍然可读**——它关掉的是监控,不是历史。

关于 `end` 有一点要知道:录制和 shell 是同一个东西,因为 shao 正是靠把 shell 包在 pty 里来录制的。所以停止一个录制**必然**结束它正在录的那个 shell,不存在"shell 活着但录制停了"这种安排。它是请每个录制器干净收尾而不是直接杀掉,所以最后那段输出是被刷盘而不是丢掉。只想停掉未来的终端、放过正在跑的,用 `--keep-shells`。

打印出来的配置提供三种接入方式,用你的平台支持的那种:

- **stdio** —— 客户端自己去启动 `shao mcp`。不占端口、不用 token、不留任何常驻进程。这是默认,对大多数人来说够了。
- **Streamable HTTP**(`/mcp`)—— 当前的 URL 式传输。
- **SSE**(`/sse`)—— 更老的 HTTP+SSE 传输,但它仍然是很多托管平台**唯一**提供的那种。平台向你索要 "MCP SSE URL" 时,要的就是这个。

`shao start --port 7337` 已经把这个端点跑起来了。如果要跑一个**不录制任何东西**的端点——比如某台机器只负责把别的终端录下的内容提供出去——直接用 `serve`:

```sh
shao serve --port 7337   # 自己转后台,打印 URL,然后返回
```

它自己转后台,是因为端点打印完 URL 之后就没什么话要说了,不该占着你的终端。想让它留在前台、实时看日志,加 `--foreground`,用 Ctrl+C 结束。

```sh
shao serve --status    # 有在跑吗,跑在哪
shao serve --stop      # 从任意终端停掉它
```

转到后台的端点在启动它的那个终端关闭后依然活着,要说的话都写进 `~/.shao/serve.log`。

**永远不要按进程名去停 shao。** `taskkill /F /IM shao.exe` 或 `pkill shao` 会连带杀掉每个被录制终端背后的录制器,而录制器持有那个 shell 的 pty,**那些 shell 会跟着一起死——不相干的窗口会坏掉**。用 `shao serve --stop` 或 `shao end`,它们是请对方干净收尾。
...
如果你压根不想要启动钩子,`shao shell` 只录当前终端,别的什么都不改。

## 多个终端,一个端点

**没有任何按终端要配的东西。** 录制器是各自独立的进程,只往 `~/.shao/sessions/` 写;一个端点读这个目录,所以你录的每个终端都从它那里可见。开十个终端,你依然只有一份 MCP 配置,而且没有任何东西需要清理。

让这件事真正可用的是**会话带身份**,几个终端因此彼此可辨——就是本页开头那个列表。

工作目录是最可靠的区分依据,而工具直接接受它:问 `cwd:webapp` 就行,不用去翻会话 id。当一个子串匹配到多个会话、而且其中不止一个还在录制时,shao 会**报错并列出候选,而不是猜**。
...
| 工作目录 | shao 自己的标记,或 `OSC 7` | 需要 bash、zsh 或 PowerShell;Windows 从不发 `OSC 7` |
...
`shao start --port N` 覆盖了通常的情形,所以大多数人根本不需要敲 `shao serve`。这两个命令都存在,是因为它们站在数据的两侧:
...
没有 `start` 就没有任何可读的东西,这时 AI 问你的终端会**正确地**报告它什么都看不到。没有 `serve` 录制照旧进行,只是 URL 式客户端连不上。如果你的客户端接受了粘贴过去的 `command` 配置,它会自己启动 `shao mcp`,`serve` 就完全不需要。

## 命令

| | |
|---|---|
| `shao start [--label NAME] [--port N]` | 开:钩子 + 配置 + 录制当前终端 |
| `shao end [--keep-hook] [--keep-shells]` | 关;已录的历史保留 |
| `shao shell [--label NAME] [--quiet]` | 只录当前终端,别的什么都不改 |
| `shao run -- ./deploy.sh` | 只录一条命令 |
| `shao hook install \| uninstall \| status` | 让新开的终端自动录制 |
| `shao sessions [--all] [--live] [--hours N]` | 录了什么,带 cwd 和各自跑过的命令 |
| `shao tail [SESSION] [--lines N] [--raw]` | 自己看,不经过 AI |
| `shao last-error [SESSION]` | 最近一条失败的命令及其输出 |
| `shao mcp` | 在 stdin/stdout 上说 MCP(客户端自己跑这个) |
| `shao serve [--foreground]` | 在 HTTP 端口上提供 MCP,不录制任何东西 |
| `shao serve --status \| --stop` | 看端点在不在,或停掉它 |
| `shao mcp-config [--format …]` | 再打印一次客户端配置 |
| `shao token rotate` | 换掉 HTTP 的 bearer token |
| `shao host add \| list \| verify \| query` | 目标主机的只读访问 |
| `shao config path \| show \| init` | 配置 |
| `shao version` | 这是哪个构建 |

`--home DIR` 对每条命令都有效,用来整体移动状态目录(默认 `~/.shao` 或 `$SHAO_HOME`)。`shao help --all` 打印全部选项。
...
当一个选择器匹配到多个会话、而且其中不止一个还活着时,shao 会报错并列出它们,**而不是猜**——从错误的终端里答话比反问你要哪一个更糟。

当 shell 是 bash、zsh 或 PowerShell 时,shao 还会记录每条命令从哪开始、到哪结束、退出码是多少。这正是把*"那个为什么失败了"*从"这是最后 500 行"变成**一次查找**的原因——最近一个非零退出,和它自己的输出。其他 shell 依然被完整采集,只是缺这份精度,而且 shao 会在回答里说明这一点,不装作有。

## 使用技巧

**在长任务开始**之前**开,而不是之后。** 录制从 shell 启动的那一刻开始,所以一个已经在没录制的窗口里失败掉的构建,是找不回来的。`shao start` 只需装一次钩子,之后你打开的每个终端都被覆盖——这是**不需要你记得**的那个版本。

**按目录问。** 开着好几个终端时,"webapp 里那个构建为什么失败"就能用,因为 `cwd:webapp` 会选中它。到处复制会话 id 是慢路;而当两个还活着的会话都匹配时,shao 会拒绝猜,而不是从错误的终端里答话。

**`last-error` 比 `tail` 好用。** 在 bash、zsh、PowerShell 下 shao 知道每条命令的起止,所以"最后一条失败的命令和它自己的输出"是**一次查找**。改成去取最后 500 行,等于把解析工作丢给助手,而它有时会解析错。

**给你还会回来看的东西起名字。** `shao start --label deploy`,下周就能问 `label:deploy`——那时候工作目录早就不好记了。

**ssh 会话是录在这一侧的。** 服务器上不需要有 shao。在被录制的 shell 里跑 `ssh`,远端输出就被本地捕获了。

**缓冲区按实测调,别猜。** `shao sessions` 会打印每个会话真实的写入速率,以及它的缓冲现在能回溯多远。如果这个时长比"出问题"到"你来问"之间的间隔还短,就调大 `buffer.max_bytes`。详见 [docs/buffer-sizing.md](docs/buffer-sizing.md)。

**用它自己的命令去停它。** `shao end`,或者只停端点用 `shao serve --stop`。`taskkill /IM shao.exe` 和 `pkill shao` 会杀掉每个被录制终端背后的录制器,而录制器持有那个 shell 的 pty,**不相干的窗口会跟着一起死**。
...
`shao host verify` 是真正要紧的那一步。它**不是**读一遍配置然后宣布成功:它会真的去尝试执行一条命令、读 `/etc/passwd`、创建文件、申请一个终端、转发一个端口——**只有每一项都被拒绝才算通过**。**主机在通过之前无法被查询**,而失败的主机会被自动设回 `disabled`。

shao 从不往目标主机写任何东西。装探针需要的权限恰恰是那把只读密钥没有、也不该有的,所以 `shao host add` 生成密钥和一个安装脚本,由**你**用自己的凭据去执行。
...
默认每个会话 256 MiB raw 加 64 MiB cooked。密集的日志刷屏大约 2–5 MB/min,所以这个量能回溯大约 1–2 小时。`shao sessions` 会显示每个会话**实测**的速率和它的缓冲实际能回溯多远,方便你按真实负载来定大小。详见 [docs/buffer-sizing.md](docs/buffer-sizing.md)(英文)。

## 敏感输出

**在你屏幕上出现过的东西都在缓冲里。** 脱敏跑两遍——一遍在写入磁盘之前,一遍在任何内容返回给 AI 之前——覆盖对看起来像密钥的变量名的赋值、`Authorization` 头、AWS / GitHub / Slack / JWT 的形态、私钥块、`mysql -p`、`curl -u`,以及 URL 里的凭据。shao 写出的一切都是仅属主可读。
...
| 2 | AI 只读,在系统层强制 | `internal/probe`(封闭动词列表、不经 shell、固定 argv)+ sshd forced command;由 `shao host verify` 证明 |
...
MIT,见 [LICENSE](LICENSE)。可以自由使用、修改、分发和商用,唯一要求是保留版权
声明;不提供任何担保。
